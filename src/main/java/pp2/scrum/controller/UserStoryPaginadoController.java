...
public class UserStoryPaginadoController extends Controller {
    ...
    public UserStoryPaginadoController(Backlog backlog) {
        this(new Paginacion<UserStory>(1, 5, backlog.getList()));
    ...
    public void actualizarPaginacion() {
        paginacionActual = new Paginacion<UserStory>(
                ...
                paginacionActual.getItemsPorPagina(), backlog.getList());
    }

    public void actualizarPaginacion(List<UserStory> model) {
        paginacionActual = new Paginacion<UserStory>(
                paginacionActual.getPagina(),
                paginacionActual.getItemsPorPagina(), model);
    }
    ...
    public List<UserStory> obtenerPaginacionAnterior() {
        return paginacionActual.getPagina() == 1
                ? paginacionActual.listarPaginacion()
                : paginacionActual.paginacionAnterior();
    }

    public List<UserStory> obtenerPaginacionSiguiente() {
        return paginacionActual.getPagina() == paginacionActual
                .getPaginasTotales() ? paginacionActual.listarPaginacion()
                        : paginacionActual.paginacionSiguiente();
    }
    ...
    public boolean hasNext() {
        return isEnabled() && paginacionActual.getPagina() < paginacionActual
                .getPaginasTotales();
    }

    public boolean hasPrev() {
        return isEnabled() && paginacionActual.getPagina() > 1;
    }
