...
public class UserStoryHelperTest extends TestCase {
    ...
    public void setUp() {
        ...
        this.userStory = new UserStory("Titulo1", "Detalle1", 10,
                new CriterioAceptacion("Criterio 1"), tareas);
        miembro = new Miembro("José");

    }

    /**
     * Verifico que se pueda acceder a los datos de la user story desde el
     * helper
     */
    public void testUserStoryHelper() {
        UserStoryHelper userStoryHelper = new UserStoryHelper(this.userStory,
                this.miembro, Estado.getDefault());
        assertTrue(
                userStoryHelper.getTitulo().equals(this.userStory.getTitulo()));
        assertTrue(userStoryHelper.getDetalle()
                .equals(this.userStory.getDetalle()));
        // assertTrue(userStoryHelper.getAutor().equals(this.userStory.getAutor()));
    }

    /**
     * Modifico el modelo y verifico que el cambio se ve reflejado desde el
     * helper
     * 
     * @throws IOException
    ...
    public void testUserStoryHelperUpdateStory()
        ...
        Workbook workbook = Workbook
                .getWorkbook(new java.io.File(path + ".xls"));
        ...
        // Dos filas, una para la cabecera y otra para la única historia de
        // usuario
        ...
        assertTrue(sheet.getCell(0, 1).getContents()
                .equals(this.userStory.getId() + ""));
        assertTrue(sheet.getCell(1, 1).getContents()
                .equals(this.userStory.getTitulo()));
        assertTrue(sheet.getCell(2, 1).getContents()
                .equals(userStoryHelper.getEstado().name()));
        assertTrue(sheet.getCell(3, 1).getContents()
                .equals(this.miembro.getNombre()));
        assertTrue(sheet.getCell(4, 1).getContents()
                .equals(this.userStory.getStoryPoints() + ""));
